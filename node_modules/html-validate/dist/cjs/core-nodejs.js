...
function findFlatConfigFile(startDir) {
  ...
  for (; ; ) {
    for (const filename of FLAT_CONFIG_FILES) {
      const candidate = path__default.default.join(current, filename);
...
async function loadFlatConfigFile(filePath) {
  const url = node_url.pathToFileURL(filePath);
  const stat = await fs__default$1.default.stat(url);
  url.searchParams.append("mtime", String(stat.mtime.getTime()));
...
function matchesFile(block, rel) {
  if (block.ignores?.some((it) => path__default.default.matchesGlob(rel, it))) {
    return false;
  }
  if (!block.files) {
    return true;
  }
  return block.files.some((it) => path__default.default.matchesGlob(rel, it.includes("/") ? it : `**/${it}`));
}
...
function buildResolvedConfig(merged, original) {
  const metaTable = new core.MetaTable();
  ...
  for (const [ruleId, data] of Object.entries(merged.rules)) {
    ...
    rules.set(ruleId, [core.parseSeverity(rawSeverity), options]);
  }
  ...
  return new core.ResolvedConfig(resolvedData, original);
}
const defaultResolvers = [esmResolver()];
class FlatConfigLoader extends core.ConfigLoader {
  configFilePath;
  configCache;
  fileCache;
  /**
  ...
  async getConfigFor(filename, configOverride) {
    ...
    const configDir = path__default.default.dirname(this.configFilePath);
    const rel = path__default.default.relative(configDir, path__default.default.resolve(filename));
    if (globalIgnores.some((p) => path__default.default.matchesGlob(rel, p))) {
...
exports.FlatConfigLoader = FlatConfigLoader;
